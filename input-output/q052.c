...
void main(){
    int accessedData, extraData;
    float account;
    const float taxe = 5;
    const int defaultGB = 100;
    const float monthlyFee = 80;
    printf("Input all GB used in this month: ");
    scanf("%d", &accessedData);
    extraData = accessedData - defaultGB;
    account = monthlyFee + extraData * 5;
    printf("Total: $%.2f\n", account);
}